...
def rule_route_path(item)
  return item.identifier + "index.html"
end
...
def image_tag(uri, title, alt, width, height, cap_color="#4084e1")
  "
  
<ul class=\"media-grid\" style=\"margin-left: 100px; margin-bottom: 20px; margin-top: 20px; \">
  <li>
  <a href=\"#{uri}\" style=\"margin: auto;\" rel=\"shadowbox[Mixed]\">
  <div class=\"cssbakery_image_wrapper\" #{cap_color} cc\" style=\"width: #{width + 50}px; height: #{height + 50}px; margin: auto; \">
  <img src=\"#{uri}\" alt=\"#{alt}\" width=\"#{width}\" height=\"#{height}\" style=\"margin: auto;\">
  <div class=\"cssbakery_transbox\" style=\"margin-top: 20px; background-color: #{cap_color}; left:0px; bottom:0px; height: 38px;\"></div>
  <p style=\"margin-top: 20px; left: 0px; bottom: 0px;\">#{alt}</p>
  </div>
  </a>
  </li>
</ul>
...
def copy_static
  FileUtils.cp_r 'static/build/', 'output/static/' 
end
...
def featured_articles
  ...
  sorted_articles.each do |item|
    @feat.push(item) if item[:featured] != 0
  end
  
  @feat.sort_by { |a| a[:featured] }
  
end

def is_disqus_developer
  "var disqus_developer = 1;" if config[:developer] else nil
end
